...
def filter_df(df, balance):
    ...
    # info2 = get_sleep_info_per_patient(df)
    # W = np.sum([x[1] for x in info2])
    # S1 = np.sum([x[2] for x in info2])
    # S2 = np.sum([x[3] for x in info2])
    # SWS = np.sum([x[4] for x in info2])
    # R = np.sum([x[5] for x in info2])
    # print(W, S1, S2, SWS, R)
